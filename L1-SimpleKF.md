...
P = A @ P @ A.T + Q
```
- **方差**：表示单个状态估计的不确定程度。例如位置方差越大，说明位置估计越不可靠。
- **协方差**：表示两个状态误差之间的关联。例如位置误差和速度误差是否倾向于同方向或反方向变化。

对于状态 $x=[位置,速度]^T$，协方差矩阵为：
...
由于状态误差会和状态一样经过矩阵 $A$ 传播：

$$
e_{\text{new}}=Ae_{\text{old}}
$$

根据协方差定义 $P=E[ee^T]$，传播后的协方差为：
...
因此，$APA^T$ 表示旧的不确定性经过系统模型 $A$ 传播后的结果，$Q$ 则表示模型误差带来的额外不确定性。
